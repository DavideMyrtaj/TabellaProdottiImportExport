...
namespace tabellaLog
{
    public partial class Form1 : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            lista.Prodotti = new List<ProdottoSingolo>();
            string query = "SELECT * FROM TabellaProdotti";
            cmd = new SqlCommand(query, Conn);
            Conn.Open();

            SqlDataReader oggetto = cmd.ExecuteReader();
            while (oggetto.Read())
            {
                lista.Prodotti.Add(new ProdottoSingolo { Prodotto = oggetto["Prodotto"].ToString(), Quantità = int.Parse(oggetto["Quantità"].ToString()), Prezzo = float.Parse(oggetto["Prezzo"].ToString()) });
            }

            oggetto.Close();
            Conn.Close();
            File.WriteAllText("export.json", JsonConvert.SerializeObject(lista,Formatting.Indented));
            MessageBox.Show("Export creato");
        }

        private void button6_Click(object sender, EventArgs e)
        {
            ListaProdotti cassa = JsonConvert.DeserializeObject<ListaProdotti>(File.ReadAllText("import.json"));
            string valori = String.Join(",", cassa.Prodotti);
            string query = $"INSERT INTO TabellaProdotti (Prodotto,Quantità,Prezzo) VALUES {valori};";
            cmd = new SqlCommand(query, Conn);
            Conn.Open();
            cmd.ExecuteNonQuery();
            Conn.Close();
            MessageBox.Show("valori importati nella tabella prodotti");
    ...
    public class ProdottoSingolo
    {
        ...
        {
            string fr = $"('{Prodotto}','{Quantità}','{Prezzo.ToString().Replace(",",".")}')";
            return fr;
        }
    }
}
